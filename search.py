...
def simulated_annealing(problem, schedule=exp_schedule()):
    ...
    # Searching for the lowest value (shortest SDL) among neighbors to goal. Returning a path vs. node.
    current = Node(problem.initial)
    path = [current]
    for t in range(sys.maxsize):
        ...
        if T == 0:
            ...
            # Indicate that we have gotten stuck at a local min.
        ...
        # Compare random neighbor to the current node. If closer or we decide to take a chance, make neighbor the current node and append to the path
...
class GraphProblem(Problem):
    ...
    def value(self, state):
        """For optimization problems, each state has a value. Hill Climbing
        and related algorithms try to maximize this value."""
        return -1 * self.path_cost(None, None, None, state)  #from lecture 4 material (TSPProblem.py)      
